...
mlp_clf_cancer = MLPClassifier(hidden_layer_sizes=(50, 50), max_iter=1000, random_state=42)
mlp_clf_cancer.fit(X_entrenamiento_cancer, y_entrenamiento_cancer)
y_prediccion_mlp_cancer = mlp_clf_cancer.predict(X_prueba_cancer)
precision_hold_out_mlp_cancer = accuracy_score(y_prueba_cancer, y_prediccion_mlp_cancer)
matriz_confusion_hold_out_mlp_cancer = confusion_matrix(y_prueba_cancer, y_prediccion_mlp_cancer)

# Cross-Validation con Perceptrón Multicapa
puntuaciones_cv_mlp_cancer = cross_val_score(mlp_clf_cancer, X_cancer, y_cancer, cv=10)
precision_cross_val_mlp_cancer = puntuaciones_cv_mlp_cancer.mean()
y_prediccion_cv_mlp_cancer = cross_val_predict(mlp_clf_cancer, X_cancer, y_cancer, cv=10)
matriz_confusion_cross_val_mlp_cancer = confusion_matrix(y_cancer, y_prediccion_cv_mlp_cancer)
...
print(f'Accuracy (Hold-Out) - Perceptrón Multicapa (Cancer): {precision_hold_out_mlp_cancer}')
print(f'Matriz de Confusión (Hold-Out) - Perceptrón Multicapa (Cancer):\n{matriz_confusion_hold_out_mlp_cancer}')
print(f'Accuracy (Cross Validation) - Perceptrón Multicapa (Cancer): {precision_cross_val_mlp_cancer}')
print(f'Matriz de Confusión (Cross Validation) - Perceptrón Multicapa (Cancer):\n{matriz_confusion_cross_val_mlp_cancer}\n')
